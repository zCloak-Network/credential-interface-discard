...
import { ethereumChains } from "./ethereumChains";
...
async function retrieve(
  ...
  // const systemChainType = api.rpc.system.chainType
  //   ? api.rpc.system.chainType()
...
function Api({
  ...
  useEffect((): void => {
    (async () => {
      console.log(90909066);
      const provider = new WsProvider(url);

      const typesDev = getDevTypes();

      api = await ApiPromise.create({ provider });
      console.log(90909066111, typesDev);
      ...
      loadOnReady(api, injectedPromise, store, typesDev)
        .then((res) => {
